...
INSERT INTO `teachers_and_director` VALUES (1,'NO','Марат','Стамескин','Рубанкович','MarRub009','b72cceada6014e392b31731d30128ffd','труд, технология',50,'+7(951)675-44-07','marat_rub@mail.ru'),(2,'NO','Валентина','Гваделупова','Аргентинова','ValArg','1100GvAr','география',36,'+7(900)109-55-41','val_arg@inbox.ru');
...
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `views` (
  `view_names` varchar(255) NOT NULL,
  `link_names` varchar(255) NOT NULL,
  `for` enum('student','teacher','director') DEFAULT NULL,
  UNIQUE KEY `views_links_names_uindex` (`view_names`,`link_names`)
...
INSERT INTO `views` VALUES ('additionally: VJournalList, VMessageTo','Дополнительно: просмотреть классный журнал, отправить сообщение','director'),('journal_list: VList','Классный журнал: оценки','student'),('journal_list: VList, VAdd, VDelete','Классный журнал: просмотр дневника, поставить оценку, удалить оценку','teacher'),('students: VList, VAdd, VDelete','Ученики: список, добавление, удаление','director'),('subjects: VList, VAdd, VDelete','Предметы: список, добавление, удаление','director'),('teachers: VList, VAdd, VDelete','Учителя: список, добавление, увольнение','director'),('test: test, test2','тест: тест, тест2','student');
...
-- Dump completed on 2018-09-04 20:29:19
